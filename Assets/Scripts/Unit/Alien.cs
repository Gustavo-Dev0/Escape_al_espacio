...
public class Alien : Unit
{
    ...
    protected override void OnUpdate()
    {
        ...
        //base.MoveTo(new Vector3(0, 0, 0));

        /*WAypoints*/
        Debug.Log("CAntidad de wp "+base.waypoints.Length);

        if (Vector3.Distance(this.transform.position, waypoints[currentWP]) < 1) currentWP++;

        if (currentWP >= waypoints.Length)  currentWP = 0;

        base.MoveTo(waypoints[currentWP]);
